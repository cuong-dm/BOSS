...
from design_baselines.coms_cleaned_BOSS.trainers import ConservativeObjectiveModel
from design_baselines.coms_cleaned_BOSS.trainers import VAETrainer
from design_baselines.coms_cleaned_BOSS.nets import ForwardModel
from design_baselines.coms_cleaned_BOSS.nets import SequentialVAE
from design_baselines.coms_cleaned_BOSS.nets import PhiGammaModel
...
@click.command()
@click.option('--path-hacked-task',
              default='coms-cleaned', type=str,
              help='The directory in which task is hacked ')
@click.option('--omega-mu-init',
              type=float,
              help='omega_mu_init')
@click.option('--omega-sigma-init',
              type=float,
              help='omega_sigma_init')
@click.option('--omega-mu-bound',
              type=float,
              help='omega_mu_bound')
@click.option('--omega-sigma-lower',
              type=float,
              help='omega_sigma_lower')
@click.option('--omega-sigma-upper',
              type=float,
              help='omega_sigma_upper')
@click.option('--lambda_',
              type=float,
              help='lambda_')
@click.option('--n-gamma',
              type=float,
              help='n_gamma')
@click.option('--lr-omega',
              type=float,
              help='lr_omega')
@click.option('--alpha-rema',
              type=float,
              help='alpha_rema')
...
def coms_cleaned_BOSS(
    ...
    omega_0 = tf.constant([[params["omega_mu_init"], params["omega_sigma_init"]]])
    lambda_ = params["lambda_"]
    n_gamma = params["n_gamma"]
    lr_omega = params["lr_omega"]
    alpha_rema = params["alpha_rema"]
    omega_mu_bound = params["omega_mu_bound"]
    omega_sigma_lower = params["omega_sigma_lower"]
    omega_sigma_upper = params["omega_sigma_upper"]
    
    # make a trainer for the forward model
    trainer = ConservativeObjectiveModel(
        phi_gamma_model, omega_0,
